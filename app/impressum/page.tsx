import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Impressum",
  alternates: {
    canonical: "/impressum",
  },
};

export default function Impressum() {
  return (
    <div className="text-foreground p-8 pt-24 ">
      <h1>Impressum</h1>
      <p>Informationen über den Diensteanbieter.</p>
      <p>
        <strong>Zeitlhofer Alex</strong>
      </p>
      <p>Alexander Zeitlhofer</p>
      <p>
        Oberschweinz 11, <br />
        3281 Oberndorf an der Melk, <br />
        Österreich
      </p>
      <p>
        <strong>Tel.:</strong> 0660 6796469
        <br />
        <strong>E-Mail:</strong>{" "}
        <a href="mailto:zeitlhoferalex05@gmail.com">
          zeitlhoferalex05@gmail.com
        </a>
      </p>
      <p>
        <strong>Unternehmensgegenstand:</strong> Webentwicklung &amp; Webdesign
        <br />
        <strong>GLN (Global Location Number):</strong> 9110038703604
        <br />
        <strong>GISA (Gewerbeinformationssystem Austria):</strong> 39161963
        <br />
        <strong>Mitglied bei:</strong> WKO Scheibbs
      </p>
      <p>
        <strong>Aufsichtsbehörde:</strong>
        <br />
        Bezirkshauptmannschaft Scheibbs
        <br />
        Rathausplatz 5, 3270 Scheibbs
        <br />
        <strong>Webseite:</strong>{" "}
        <a
          href="https://www.noe.gv.at/noe/Scheibbs/Bezirkshauptmannschaft_Scheibbs.html"
          target="_blank"
          rel="noopener">
          https://www.noe.gv.at/noe/Scheibbs/Bezirkshauptmannschaft_Scheibbs.html
        </a>
      </p>
      <p>
        <strong>Berufsbezeichnung:</strong> Webentwickler
        <br />
        <strong>Verleihungsstaat:</strong> Österreich
      </p>
      <p>
        <strong>Datenschutz Verantwortlicher</strong>
        <br />
        Alexander Zeitlhofer
        <br />
        Oberschweinz 11,
        <br />
        3281 Oberndorf an der Melk
      </p>
      <p>Alle Texte sind urheberrechtlich geschützt.</p>
      <p className="mt-4">
        Quelle: Erstellt mit dem{" "}
        <a
          href="https://www.adsimple.at/impressum-generator/"
          title="Impressum Generator Österreich von AdSimple">
          Impressum Generator
        </a>{" "}
        von AdSimple
      </p>
      <h2>Icons:</h2>
      <a href="https://www.flaticon.com/free-icons/html" title="html icons">
        Html icons created by Freepik - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/css" title="css icons">
        Css icons created by iconsimple - Flaticon
      </a>
      <br></br>
      <a
        href="https://www.flaticon.com/free-icons/javascript"
        title="javascript icons">
        Javascript icons created by Freepik - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/react" title="react icons">
        React icons created by Erix - Flaticon
      </a>
      <br></br>
      <a
        href="https://www.flaticon.com/free-icons/node-js"
        title="node js icons">
        Node js icons created by Freepik - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/java" title="java icons">
        Java icons created by Freepik - Flaticon
      </a>
      <br></br>
      <a
        href="https://www.flaticon.com/free-icons/c-sharp"
        title="c sharp icons">
        C sharp icons created by Freepik - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/svelte" title="svelte icons">
        Svelte icons created by iconizerapp - Flaticon
      </a>
      <br></br>
      <a
        href="https://www.flaticon.com/free-icons/contact"
        title="contact icons">
        Contact icons created by raanpooo - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/phone" title="phone icons">
        Phone icons created by Dave Gandy - Flaticon
      </a>
      <br></br>
      <a
        href="https://www.flaticon.com/free-icons/instagram-logo"
        title="instagram logo icons">
        Instagram logo icons created by Pixel perfect - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/github" title="github icons">
        Github icons created by Pixel perfect - Flaticon
      </a>
      <br></br>
      <a href="https://www.flaticon.com/free-icons/email" title="email icons">
        Email icons created by Freepik - Flaticon
      </a>
      <br></br>
      <a
        href="https://www.flaticon.com/free-icons/linkedin"
        title="linkedin icons">
        Linkedin icons created by riajulislam - Flaticon
      </a>
      <br></br>
      Sound Effect by{" "}
      <a href="https://pixabay.com/users/matthewvakaliuk73627-48347364/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=290204">
        Matthew Vakalyuk
      </a>{" "}
      from{" "}
      <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=290204">
        Pixabay
      </a>
      <p>
        Photo by Codioful (formerly Gradienta) from Pexels:
        https://www.pexels.com/photo/colorful-gradient-6985001/
      </p>
    </div>
  );
}
